$(function(){
				//获取大盒子
				var wraper1 = $('#wraper_court');
				//获取图片ul
				var pics1 = $('#pics_court');
				//获取所有图片
				var picL = pics1.children('li');
				//获取所有的点
				var dotL = $('#dots_court').children('li');
				//获取每张图片的高度
				var picW = picL.width();
				
				//定义一个计时器，使图片循环，计时器作为属性添加到wraper中，防止被其他因素影响到
//				wraper1[0].toRun = setInterval(play,2000)
				
				function play(){
					//图片往上跑，所以ul的top每次减小picH个距离
					var target = pics1.position().left - picW;
					//当最后一张图片出现在屏幕时，继续跑，不要停，并且把第一张定位到最后一张的后面来
					if(pics1.position().left <= -(picL.length-1)*picW){
						picL.eq(0).css('left',picL.length*picW)
					}
					//当第一张图片也出现在屏幕中的时候，还原第一张图片的位置，同时让pics的top值归0
					if (pics1.position().left <= -picL.length*picW) {
						picL.eq(0).css('left',0);
						pics1.css('left',0);
						//因为当前位置已经改变了，重新获取target
						target = pics1.position().left - picW;
					}
					console.log(target)
					//调用animate方法，执行动画
					pics1.animate({
